# Organisation de fireviewer-fire-state

Source canonique : **[fireviewer/fireviewer-fire-state](https://github.com/fireviewer/fireviewer-fire-state)**. Responsabilité technique : **FV**. Accès : **private**.

Calcul Part.4, profils, calibration et évaluation.

## Où travailler et commiter

Dans le workspace organisé, ouvrir **`actifs/fireviewer/fireviewer-fire-state`**. Chaque dossier est un dépôt Git autonome. Créer une branche de chantier dans ce dépôt ; pour un travail parallèle, créer un worktree sous `travaux/<chantier>/<composant>`. Ne jamais commiter depuis la racine multi-dépôts, `archives/` ou un ancien chemin de compatibilité.

Fusion et calcul de l’état incendie sur entrées explicites.

Transactions, révisions et stockage restent au backend ; le calcul seul n’est pas le Perimeter Builder général.
